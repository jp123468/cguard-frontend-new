...
import {
  ...
  RondaSettings,
} from "@/lib/api/rondaSettingsService";
...
function NumberRow({
  label, hint, value, onChange, min = 0, suffix,
}: {
...
function Content() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      const res = await rondaSettingsService.update(s);
      setS({ ...DEFAULTS, ...res });
...
export default function RondasSettingsPage() {
  return (
    <AppLayout>
      <SettingsLayout title="Configuraciones de Rondas">
        <Content />
      </SettingsLayout>
    </AppLayout>
  );
