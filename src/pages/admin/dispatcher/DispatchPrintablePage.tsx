  ...
  return (
    <div className="mt-6 space-y-5">
              ...
              <table className="w-full text-sm border-collapse">
                <tbody>
                  <tr className="border-b">
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.ticket_id') || 'Ticket ID'}</td>
                    <td className="py-3 px-4 text-sm text-foreground">{ticketShort} <span className={`inline-block text-xs ml-2 px-2 py-0.5 rounded-full ${requestPayload.status === 'open' || requestPayload.status === 'abierto' ? 'bg-red-500/15 text-red-700' : 'bg-green-100 text-green-700'}`}>{requestPayload.status ?? ''}</span></td>
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.date_time') || 'Date/Time'}</td>
                  ...
                  <tr className="border-b">
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.caller_type') || 'Caller Type'}</td>
                    <td className="py-3 px-4 text-sm text-foreground">{requestPayload.callerType || '-'}</td>
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.caller_name') || 'Caller Name'}</td>
                    <td className="py-3 px-4 text-sm text-foreground">{requestPayload.callerName || '-'}</td>
                  </tr>

                  <tr className="border-b">
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.incident_type_label') || 'Incident Type'}</td>
                    <td className="py-3 px-4 text-sm text-foreground">{incidentTypeDisplay}</td>
                    <td className="py-3 px-4 text-xs font-medium text-muted-foreground">{t('dispatcher.incident_date_time_label') || 'Incident Date/Time'}</td>
            ...
            <div className="p-4">
              ...
              <div className="mt-3">
                <div className="border rounded px-3 py-2 bg-muted/30">
                  <div className="text-xs text-muted-foreground mb-2">{t('dispatcher.comment.attach_file_optional') || 'Adjuntar archivo (opcional)'}</div>
      ...
      <div className="mt-4 space-y-4">
        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.incident_details') || 'Incident Details'}</div>
          <div className="p-4 text-sm">{requestPayload?.content || requestPayload?.incidentDetails || requestPayload?.details || '-'}</div>
        </div>

        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.action_taken') || 'Action Taken'}</div>
          <div className="p-4 text-sm">{requestPayload?.actionsTaken || requestPayload?.actionTaken || requestPayload?.action || '-'}</div>
        </div>

        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.internal_notes') || 'Internal Notes'}</div>
          <div className="p-4 text-sm whitespace-pre-wrap">{requestPayload?.internalNotes || requestPayload?.notes || '-'}</div>
        </div>

        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.files') || 'Files'}</div>
        ...
        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.guard_assigned') || 'Guard Assigned'}</div>
          <div className="p-4 text-sm">
      ...
      <div className="mt-6 max-w-4xl mx-auto">
        <div className="bg-card border rounded-md overflow-hidden">
          <div className="bg-muted px-4 py-2 text-sm font-semibold">{t('dispatcher.ticket_summary') || 'Ticket Summary'}</div>
          <div className="p-4">
            {comments.length === 0 ? (
              <div className="text-sm text-muted-foreground">{t('dispatcher.no_comments_yet') || 'No hay comentarios aún'}</div>
            ) : (
