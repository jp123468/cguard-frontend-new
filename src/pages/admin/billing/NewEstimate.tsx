...
export default function NewEstimate() {
    ...
    // Services (items) cargados desde el backend para seleccionar en los artículos
    ...
    // Load tenant basic info to show in estimate header/footer
    useEffect(() => {
        let mounted = true;
        (async () => {
            try {
    ...
    // Ensure dates are within allowed ranges: date <= today, expiryDate >= today
    useEffect(() => {
        const today = new Date();
        const todayStart = new Date(today.getFullYear(), today.getMonth(), today.getDate());
        if (date && date > todayStart) setDate(todayStart);
    ...
    const handlePreview = () => {
        ...
        if (date && date > todayStart) {
            toast.error('La fecha seleccionada no puede ser posterior a hoy');
            return;
        }
        if (expiryDate && expiryDate < todayStart) {
            toast.error('La fecha seleccionada no puede ser anterior a hoy');
            return;
        }
        if (!client || !site) {
                    ...
                    <div className="bg-white p-8 rounded-lg border shadow-sm space-y-8">
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                ...
                                <h3 className="font-semibold text-lg text-slate-700 mt-2">{tenantInfo?.name ?? ''}</h3>
                            ...
                            <div>
                                ...
                                <div className="space-y-1">
                                    {(() => {
                                        ...
                                        if (rawC) {
                                            if (rawC.companyAddress) addrCandidates.push(rawC.companyAddress);
                                            if (rawC.address) addrCandidates.push(typeof rawC.address === 'string' ? rawC.address : [rawC.address.street, rawC.address.city, rawC.address.state, rawC.address.postalCode].filter(Boolean).join(', '));
                                            if (rawC.location) addrCandidates.push(rawC.location);
                                        }
                                        if (rawS) {
                                            if (rawS.address) addrCandidates.push(typeof rawS.address === 'string' ? rawS.address : [rawS.address.street, rawS.address.city].filter(Boolean).join(', '));
                                            if (rawS.location) addrCandidates.push(rawS.location);
                                        }
                                        ...
                                        return (
                                            <>
                                                ...
                                                {postName || postAddress || postPhone || postEmail ? (
                                                    <div className="mt-2">
                                                        {postName ? <p className="text-sm font-medium text-slate-700">{postName}</p> : null}
                                                        {postAddress ? <p className="text-sm text-slate-500">{postAddress}</p> : null}
                                                        {postPhone ? <p className="text-sm text-slate-500">{postPhone}</p> : null}
                                                        {postEmail ? <p className="text-sm text-slate-500">{postEmail}</p> : null}
                                                    </div>
                                                ) : null}
                                            </>
        ...
        <AppLayout>
            <div className="p-6">
                ...
                <div className="space-y-6 bg-white p-6 rounded-lg border shadow-sm">
                    ...
                    {/* Static Company Info (from tenant) */}
                    <div className="py-4">
                        <h3 className="font-medium text-lg text-slate-700">{tenantInfo?.name ?? 'Seguridad BAS'}</h3>
                        <p className="text-sm text-slate-500">{tenantInfo?.address ?? 'Antonio Miguel de solier N29-26 y bartolome de las casas'}</p>
                        <p className="text-sm text-slate-500">{tenantInfo?.phone ?? '+18014004269'}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                            <h3 className="font-medium text-lg text-slate-700">Facturar a</h3>
                                ...
                                {/* Inline preview panel: moved so it appears below the site selector */}
                                    ...
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            ...
                                            <Button
                                                ...
                                                title={items.length <= 1 ? "No se puede eliminar el último artículo" : "Eliminar artículo"}
                                                className={cn("text-red-500 hover:text-red-700 hover:bg-red-50", items.length <= 1 ? "opacity-40 cursor-not-allowed hover:bg-transparent hover:text-red-500" : "")}
                                            >
